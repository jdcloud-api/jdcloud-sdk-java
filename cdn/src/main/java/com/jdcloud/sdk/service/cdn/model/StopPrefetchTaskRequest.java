/*
 * Copyright 2018 JDCLOUD.COM
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http:#www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * Pcdn刷新预热接口
 * Openapi For JCLOUD cdn
 *
 * OpenAPI spec version: v1
 * Contact: pid-cdn@jd.com
 *
 * NOTE: This class is auto generated by the jdcloud code generator program.
 */

package com.jdcloud.sdk.service.cdn.model;

import java.util.List;
import java.util.ArrayList;
import com.jdcloud.sdk.service.JdcloudRequest;

/**
 * 停止预热任务接口
 */
public class StopPrefetchTaskRequest extends JdcloudRequest implements java.io.Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 待停止预热的url
     */
    private List<String> urls;

    /**
     * 地区[huabei huadong dongbei huazhong huanan xinan xibei gangaotai]中的一个
     */
    private String region;

    /**
     * 运营商[ct uni cm]中的一个,分别代表电信 联通 移动
     */
    private String isp;


    /**
     * get 待停止预热的url
     *
     * @return
     */
    public List<String> getUrls() {
        return urls;
    }

    /**
     * set 待停止预热的url
     *
     * @param urls
     */
    public void setUrls(List<String> urls) {
        this.urls = urls;
    }

    /**
     * get 地区[huabei huadong dongbei huazhong huanan xinan xibei gangaotai]中的一个
     *
     * @return
     */
    public String getRegion() {
        return region;
    }

    /**
     * set 地区[huabei huadong dongbei huazhong huanan xinan xibei gangaotai]中的一个
     *
     * @param region
     */
    public void setRegion(String region) {
        this.region = region;
    }

    /**
     * get 运营商[ct uni cm]中的一个,分别代表电信 联通 移动
     *
     * @return
     */
    public String getIsp() {
        return isp;
    }

    /**
     * set 运营商[ct uni cm]中的一个,分别代表电信 联通 移动
     *
     * @param isp
     */
    public void setIsp(String isp) {
        this.isp = isp;
    }


    /**
     * set 待停止预热的url
     *
     * @param urls
     */
    public StopPrefetchTaskRequest urls(List<String> urls) {
        this.urls = urls;
        return this;
    }

    /**
     * set 地区[huabei huadong dongbei huazhong huanan xinan xibei gangaotai]中的一个
     *
     * @param region
     */
    public StopPrefetchTaskRequest region(String region) {
        this.region = region;
        return this;
    }

    /**
     * set 运营商[ct uni cm]中的一个,分别代表电信 联通 移动
     *
     * @param isp
     */
    public StopPrefetchTaskRequest isp(String isp) {
        this.isp = isp;
        return this;
    }


    /**
     * add item to 待停止预热的url
     *
     * @param url
     */
    public void addUrl(String url) {
        if (this.urls == null) {
            this.urls = new ArrayList<>();
        }
        this.urls.add(url);
    }

}