/*
 * Copyright 2018 JDCLOUD.COM
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http:#www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * SCDN相关接口
 * Openapi For JCLOUD cdn
 *
 * OpenAPI spec version: v1
 * Contact: pid-cdn@jd.com
 *
 * NOTE: This class is auto generated by the jdcloud code generator program.
 */

package com.jdcloud.sdk.service.cdn.model;

import com.jdcloud.sdk.annotation.Required;
import com.jdcloud.sdk.service.JdcloudRequest;

/**
 * 修改指定的黑名单规则
 */
public class UpdateWafBlackRuleRequest extends JdcloudRequest implements java.io.Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 黑名单类型， uri ip geo
     */
    private String ruleType;

    /**
     * 匹配模式,uri类型有效，0&#x3D;完全匹配  1&#x3D;前缀匹配 2&#x3D;包含 3&#x3D;正则 4&#x3D;大于 5&#x3D;后缀
     */
    private Integer matchOp;

    /**
     * 匹配值
     */
    private String val;

    /**
     * 1：forbidden，493封禁并返回自定义页面 2：redirect，302跳转 3： verify@captcha 4： verify@jscookie
     */
    private Integer atOp;

    /**
     * action为1时为自定义页面名称,空值或缺省值default为默认页面，2时为跳转url，其他时无效
     */
    private String atVal;

    /**
     * 用户域名
     * Required:true
     */
    @Required
    private String domain;

    /**
     * 需要修改的黑名单ID
     * Required:true
     */
    @Required
    private String id;


    /**
     * get 黑名单类型， uri ip geo
     *
     * @return
     */
    public String getRuleType() {
        return ruleType;
    }

    /**
     * set 黑名单类型， uri ip geo
     *
     * @param ruleType
     */
    public void setRuleType(String ruleType) {
        this.ruleType = ruleType;
    }

    /**
     * get 匹配模式,uri类型有效，0&#x3D;完全匹配  1&#x3D;前缀匹配 2&#x3D;包含 3&#x3D;正则 4&#x3D;大于 5&#x3D;后缀
     *
     * @return
     */
    public Integer getMatchOp() {
        return matchOp;
    }

    /**
     * set 匹配模式,uri类型有效，0&#x3D;完全匹配  1&#x3D;前缀匹配 2&#x3D;包含 3&#x3D;正则 4&#x3D;大于 5&#x3D;后缀
     *
     * @param matchOp
     */
    public void setMatchOp(Integer matchOp) {
        this.matchOp = matchOp;
    }

    /**
     * get 匹配值
     *
     * @return
     */
    public String getVal() {
        return val;
    }

    /**
     * set 匹配值
     *
     * @param val
     */
    public void setVal(String val) {
        this.val = val;
    }

    /**
     * get 1：forbidden，493封禁并返回自定义页面 2：redirect，302跳转 3： verify@captcha 4： verify@jscookie
     *
     * @return
     */
    public Integer getAtOp() {
        return atOp;
    }

    /**
     * set 1：forbidden，493封禁并返回自定义页面 2：redirect，302跳转 3： verify@captcha 4： verify@jscookie
     *
     * @param atOp
     */
    public void setAtOp(Integer atOp) {
        this.atOp = atOp;
    }

    /**
     * get action为1时为自定义页面名称,空值或缺省值default为默认页面，2时为跳转url，其他时无效
     *
     * @return
     */
    public String getAtVal() {
        return atVal;
    }

    /**
     * set action为1时为自定义页面名称,空值或缺省值default为默认页面，2时为跳转url，其他时无效
     *
     * @param atVal
     */
    public void setAtVal(String atVal) {
        this.atVal = atVal;
    }

    /**
     * get 用户域名
     *
     * @return
     */
    public String getDomain() {
        return domain;
    }

    /**
     * set 用户域名
     *
     * @param domain
     */
    public void setDomain(String domain) {
        this.domain = domain;
    }

    /**
     * get 需要修改的黑名单ID
     *
     * @return
     */
    public String getId() {
        return id;
    }

    /**
     * set 需要修改的黑名单ID
     *
     * @param id
     */
    public void setId(String id) {
        this.id = id;
    }


    /**
     * set 黑名单类型， uri ip geo
     *
     * @param ruleType
     */
    public UpdateWafBlackRuleRequest ruleType(String ruleType) {
        this.ruleType = ruleType;
        return this;
    }

    /**
     * set 匹配模式,uri类型有效，0&#x3D;完全匹配  1&#x3D;前缀匹配 2&#x3D;包含 3&#x3D;正则 4&#x3D;大于 5&#x3D;后缀
     *
     * @param matchOp
     */
    public UpdateWafBlackRuleRequest matchOp(Integer matchOp) {
        this.matchOp = matchOp;
        return this;
    }

    /**
     * set 匹配值
     *
     * @param val
     */
    public UpdateWafBlackRuleRequest val(String val) {
        this.val = val;
        return this;
    }

    /**
     * set 1：forbidden，493封禁并返回自定义页面 2：redirect，302跳转 3： verify@captcha 4： verify@jscookie
     *
     * @param atOp
     */
    public UpdateWafBlackRuleRequest atOp(Integer atOp) {
        this.atOp = atOp;
        return this;
    }

    /**
     * set action为1时为自定义页面名称,空值或缺省值default为默认页面，2时为跳转url，其他时无效
     *
     * @param atVal
     */
    public UpdateWafBlackRuleRequest atVal(String atVal) {
        this.atVal = atVal;
        return this;
    }

    /**
     * set 用户域名
     *
     * @param domain
     */
    public UpdateWafBlackRuleRequest domain(String domain) {
        this.domain = domain;
        return this;
    }

    /**
     * set 需要修改的黑名单ID
     *
     * @param id
     */
    public UpdateWafBlackRuleRequest id(String id) {
        this.id = id;
        return this;
    }


}