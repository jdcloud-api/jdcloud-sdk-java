/*
 * Copyright 2018 JDCLOUD.COM
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http:#www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * 
 * 
 *
 * 
 * Contact: 
 *
 * NOTE: This class is auto generated by the jdcloud code generator program.
 */

package com.jdcloud.sdk.service.vpc.model;

import java.util.List;
import java.util.ArrayList;
import com.jdcloud.sdk.service.charge.model.Charge;

/**
 * elasticIpInternal
 */
public class ElasticIpInternal  implements java.io.Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 弹性公网IP的Id
     */
    private String elasticIpId;

    /**
     * 弹性公网IP的地址
     */
    private String elasticIpAddress;

    /**
     * 弹性公网IP的限速（单位：Mbps)
     */
    private Integer bandwidthMbps;

    /**
     * 弹性公网IP的线路，标准公网IP的线路、取值为bgp或no_bgp；边缘公网IP的线路、可通过describeEdgeIpProviders接口获取
     */
    private String provider;

    /**
     * 私有IP的IPV4地址
     */
    private String privateIpAddress;

    /**
     * 配置弹性网卡Id
     */
    private String networkInterfaceId;

    /**
     * 实例Id
     */
    private String instanceId;

    /**
     * 实例类型,取值为：compute、lb、container、pod、natgw
     */
    private String instanceType;

    /**
     * 计费配置
     */
    private Charge charge;

    /**
     * 弹性公网IP的创建时间
     */
    private String createdTime;

    /**
     * 弹性公网IP的可用区属性，如果为空，表示全可用区
     */
    private String az;

    /**
     * Tag信息
     */
    
    private List<Tag> tags;
    /**
     * 弹性公网IP的IP类型，取值：standard(标准弹性IP)、edge(边缘弹性IP)
     */
    private String ipType;

    /**
     * 加入的共享带宽包ID，如果没有加入共享带宽包该值为空
     */
    private String bandwidthPackageId;

    /**
     * IP是否被绑定，取值：ASSOCIATED（被绑定）、NOT_ASSOCIATED（未被绑定）
     */
    private String status;

    /**
     * IP是否变更，取值：UP（正常使用）、DOWN（停止使用）、PROCESSING（配置变更中）
     */
    private String state;

    /**
     * 资源所属资源组ID
     */
    private String resourceGroupId;



    /**
     * get 弹性公网IP的Id
     *
     * @return
     */
    public String getElasticIpId() {
        return elasticIpId;
    }

    /**
     * set 弹性公网IP的Id
     *
     * @param elasticIpId
     */
    public void setElasticIpId(String elasticIpId) {
        this.elasticIpId = elasticIpId;
    }


    /**
     * get 弹性公网IP的地址
     *
     * @return
     */
    public String getElasticIpAddress() {
        return elasticIpAddress;
    }

    /**
     * set 弹性公网IP的地址
     *
     * @param elasticIpAddress
     */
    public void setElasticIpAddress(String elasticIpAddress) {
        this.elasticIpAddress = elasticIpAddress;
    }


    /**
     * get 弹性公网IP的限速（单位：Mbps)
     *
     * @return
     */
    public Integer getBandwidthMbps() {
        return bandwidthMbps;
    }

    /**
     * set 弹性公网IP的限速（单位：Mbps)
     *
     * @param bandwidthMbps
     */
    public void setBandwidthMbps(Integer bandwidthMbps) {
        this.bandwidthMbps = bandwidthMbps;
    }


    /**
     * get 弹性公网IP的线路，标准公网IP的线路、取值为bgp或no_bgp；边缘公网IP的线路、可通过describeEdgeIpProviders接口获取
     *
     * @return
     */
    public String getProvider() {
        return provider;
    }

    /**
     * set 弹性公网IP的线路，标准公网IP的线路、取值为bgp或no_bgp；边缘公网IP的线路、可通过describeEdgeIpProviders接口获取
     *
     * @param provider
     */
    public void setProvider(String provider) {
        this.provider = provider;
    }


    /**
     * get 私有IP的IPV4地址
     *
     * @return
     */
    public String getPrivateIpAddress() {
        return privateIpAddress;
    }

    /**
     * set 私有IP的IPV4地址
     *
     * @param privateIpAddress
     */
    public void setPrivateIpAddress(String privateIpAddress) {
        this.privateIpAddress = privateIpAddress;
    }


    /**
     * get 配置弹性网卡Id
     *
     * @return
     */
    public String getNetworkInterfaceId() {
        return networkInterfaceId;
    }

    /**
     * set 配置弹性网卡Id
     *
     * @param networkInterfaceId
     */
    public void setNetworkInterfaceId(String networkInterfaceId) {
        this.networkInterfaceId = networkInterfaceId;
    }


    /**
     * get 实例Id
     *
     * @return
     */
    public String getInstanceId() {
        return instanceId;
    }

    /**
     * set 实例Id
     *
     * @param instanceId
     */
    public void setInstanceId(String instanceId) {
        this.instanceId = instanceId;
    }


    /**
     * get 实例类型,取值为：compute、lb、container、pod、natgw
     *
     * @return
     */
    public String getInstanceType() {
        return instanceType;
    }

    /**
     * set 实例类型,取值为：compute、lb、container、pod、natgw
     *
     * @param instanceType
     */
    public void setInstanceType(String instanceType) {
        this.instanceType = instanceType;
    }


    /**
     * get 计费配置
     *
     * @return
     */
    public Charge getCharge() {
        return charge;
    }

    /**
     * set 计费配置
     *
     * @param charge
     */
    public void setCharge(Charge charge) {
        this.charge = charge;
    }


    /**
     * get 弹性公网IP的创建时间
     *
     * @return
     */
    public String getCreatedTime() {
        return createdTime;
    }

    /**
     * set 弹性公网IP的创建时间
     *
     * @param createdTime
     */
    public void setCreatedTime(String createdTime) {
        this.createdTime = createdTime;
    }


    /**
     * get 弹性公网IP的可用区属性，如果为空，表示全可用区
     *
     * @return
     */
    public String getAz() {
        return az;
    }

    /**
     * set 弹性公网IP的可用区属性，如果为空，表示全可用区
     *
     * @param az
     */
    public void setAz(String az) {
        this.az = az;
    }


    /**
    * get Tag信息
    *
    * @return
    */
    public List<Tag> getTags() {
        return tags;
    }

    /**
    * set Tag信息
    *
    * @param tags
    */
    public void setTags(List<Tag> tags) {
        this.tags = tags;
    }


    /**
     * get 弹性公网IP的IP类型，取值：standard(标准弹性IP)、edge(边缘弹性IP)
     *
     * @return
     */
    public String getIpType() {
        return ipType;
    }

    /**
     * set 弹性公网IP的IP类型，取值：standard(标准弹性IP)、edge(边缘弹性IP)
     *
     * @param ipType
     */
    public void setIpType(String ipType) {
        this.ipType = ipType;
    }


    /**
     * get 加入的共享带宽包ID，如果没有加入共享带宽包该值为空
     *
     * @return
     */
    public String getBandwidthPackageId() {
        return bandwidthPackageId;
    }

    /**
     * set 加入的共享带宽包ID，如果没有加入共享带宽包该值为空
     *
     * @param bandwidthPackageId
     */
    public void setBandwidthPackageId(String bandwidthPackageId) {
        this.bandwidthPackageId = bandwidthPackageId;
    }


    /**
     * get IP是否被绑定，取值：ASSOCIATED（被绑定）、NOT_ASSOCIATED（未被绑定）
     *
     * @return
     */
    public String getStatus() {
        return status;
    }

    /**
     * set IP是否被绑定，取值：ASSOCIATED（被绑定）、NOT_ASSOCIATED（未被绑定）
     *
     * @param status
     */
    public void setStatus(String status) {
        this.status = status;
    }


    /**
     * get IP是否变更，取值：UP（正常使用）、DOWN（停止使用）、PROCESSING（配置变更中）
     *
     * @return
     */
    public String getState() {
        return state;
    }

    /**
     * set IP是否变更，取值：UP（正常使用）、DOWN（停止使用）、PROCESSING（配置变更中）
     *
     * @param state
     */
    public void setState(String state) {
        this.state = state;
    }


    /**
     * get 资源所属资源组ID
     *
     * @return
     */
    public String getResourceGroupId() {
        return resourceGroupId;
    }

    /**
     * set 资源所属资源组ID
     *
     * @param resourceGroupId
     */
    public void setResourceGroupId(String resourceGroupId) {
        this.resourceGroupId = resourceGroupId;
    }



    /**
     * set 弹性公网IP的Id
     *
     * @param elasticIpId
     */
    public ElasticIpInternal elasticIpId(String elasticIpId) {
        this.elasticIpId = elasticIpId;
        return this;
    }


    /**
     * set 弹性公网IP的地址
     *
     * @param elasticIpAddress
     */
    public ElasticIpInternal elasticIpAddress(String elasticIpAddress) {
        this.elasticIpAddress = elasticIpAddress;
        return this;
    }


    /**
     * set 弹性公网IP的限速（单位：Mbps)
     *
     * @param bandwidthMbps
     */
    public ElasticIpInternal bandwidthMbps(Integer bandwidthMbps) {
        this.bandwidthMbps = bandwidthMbps;
        return this;
    }


    /**
     * set 弹性公网IP的线路，标准公网IP的线路、取值为bgp或no_bgp；边缘公网IP的线路、可通过describeEdgeIpProviders接口获取
     *
     * @param provider
     */
    public ElasticIpInternal provider(String provider) {
        this.provider = provider;
        return this;
    }


    /**
     * set 私有IP的IPV4地址
     *
     * @param privateIpAddress
     */
    public ElasticIpInternal privateIpAddress(String privateIpAddress) {
        this.privateIpAddress = privateIpAddress;
        return this;
    }


    /**
     * set 配置弹性网卡Id
     *
     * @param networkInterfaceId
     */
    public ElasticIpInternal networkInterfaceId(String networkInterfaceId) {
        this.networkInterfaceId = networkInterfaceId;
        return this;
    }


    /**
     * set 实例Id
     *
     * @param instanceId
     */
    public ElasticIpInternal instanceId(String instanceId) {
        this.instanceId = instanceId;
        return this;
    }


    /**
     * set 实例类型,取值为：compute、lb、container、pod、natgw
     *
     * @param instanceType
     */
    public ElasticIpInternal instanceType(String instanceType) {
        this.instanceType = instanceType;
        return this;
    }


    /**
     * set 计费配置
     *
     * @param charge
     */
    public ElasticIpInternal charge(Charge charge) {
        this.charge = charge;
        return this;
    }


    /**
     * set 弹性公网IP的创建时间
     *
     * @param createdTime
     */
    public ElasticIpInternal createdTime(String createdTime) {
        this.createdTime = createdTime;
        return this;
    }


    /**
     * set 弹性公网IP的可用区属性，如果为空，表示全可用区
     *
     * @param az
     */
    public ElasticIpInternal az(String az) {
        this.az = az;
        return this;
    }


    /**
    * set Tag信息
    *
    * @param tags
    */
    public ElasticIpInternal tags(List<Tag> tags) {
        this.tags = tags;
        return this;
    }


    /**
     * set 弹性公网IP的IP类型，取值：standard(标准弹性IP)、edge(边缘弹性IP)
     *
     * @param ipType
     */
    public ElasticIpInternal ipType(String ipType) {
        this.ipType = ipType;
        return this;
    }


    /**
     * set 加入的共享带宽包ID，如果没有加入共享带宽包该值为空
     *
     * @param bandwidthPackageId
     */
    public ElasticIpInternal bandwidthPackageId(String bandwidthPackageId) {
        this.bandwidthPackageId = bandwidthPackageId;
        return this;
    }


    /**
     * set IP是否被绑定，取值：ASSOCIATED（被绑定）、NOT_ASSOCIATED（未被绑定）
     *
     * @param status
     */
    public ElasticIpInternal status(String status) {
        this.status = status;
        return this;
    }


    /**
     * set IP是否变更，取值：UP（正常使用）、DOWN（停止使用）、PROCESSING（配置变更中）
     *
     * @param state
     */
    public ElasticIpInternal state(String state) {
        this.state = state;
        return this;
    }


    /**
     * set 资源所属资源组ID
     *
     * @param resourceGroupId
     */
    public ElasticIpInternal resourceGroupId(String resourceGroupId) {
        this.resourceGroupId = resourceGroupId;
        return this;
    }



    /**
     * add item to Tag信息
     *
     * @param tag
     */
    public void addTag(Tag tag) {
        if (this.tags == null) {
            this.tags = new ArrayList<>();
        }
        this.tags.add(tag);
    }
}