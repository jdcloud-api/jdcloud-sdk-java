/*
 * Copyright 2018 JDCLOUD.COM
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http:#www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * 用户应用分组
 * 
 *
 * OpenAPI spec version: v1
 * Contact: 
 *
 * NOTE: This class is auto generated by the jdcloud code generator program.
 */

package com.jdcloud.sdk.service.ydapp.model;

import java.util.List;
import java.util.ArrayList;
import com.jdcloud.sdk.service.ydapp.model.Group;
import com.jdcloud.sdk.service.JdcloudResult;

/**
 * 分页获取指定应用下的应用组列表
 */
public class DescribeGroupsResult extends JdcloudResult implements java.io.Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * data
     */
    
    private List<Group> data;
    /**
     * 本次查询可匹配到的总记录数，使用者需要结合 pageNumber 和 pageSize 计算是否可以继续分页。
     */
    private Long totalCount;



    /**
    * get data
    *
    * @return
    */
    public List<Group> getData() {
        return data;
    }

    /**
    * set data
    *
    * @param data
    */
    public void setData(List<Group> data) {
        this.data = data;
    }


    /**
     * get 本次查询可匹配到的总记录数，使用者需要结合 pageNumber 和 pageSize 计算是否可以继续分页。
     *
     * @return
     */
    public Long getTotalCount() {
        return totalCount;
    }

    /**
     * set 本次查询可匹配到的总记录数，使用者需要结合 pageNumber 和 pageSize 计算是否可以继续分页。
     *
     * @param totalCount
     */
    public void setTotalCount(Long totalCount) {
        this.totalCount = totalCount;
    }



    /**
    * set data
    *
    * @param data 
    * @return DescribeGroupsResult
    */
    public DescribeGroupsResult data(List<Group> data) {
        this.data = data;
        return this;
    }


    /**
     * set 本次查询可匹配到的总记录数，使用者需要结合 pageNumber 和 pageSize 计算是否可以继续分页。
     *
     * @param totalCount 本次查询可匹配到的总记录数，使用者需要结合 pageNumber 和 pageSize 计算是否可以继续分页。
     * @return DescribeGroupsResult
     */
    public DescribeGroupsResult totalCount(Long totalCount) {
        this.totalCount = totalCount;
        return this;
    }



    /**
     * add item to data
     *
     * @param data
     */
    public void addData(Group data) {
        if (this.data == null) {
            this.data = new ArrayList<>();
        }
        this.data.add(data);
    }
}