/*
 * Copyright 2018 JDCLOUD.COM
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http:#www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * 
 * 
 *
 * 
 * Contact: 
 *
 * NOTE: This class is auto generated by the jdcloud code generator program.
 */

package com.jdcloud.sdk.service.monitorcm.model;

import com.jdcloud.sdk.annotation.Required;

/**
 * metricData
 */
public class MetricData  implements java.io.Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 命名空间 ，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     * Required:true
     */
    @Required
    private String namespace;

    /**
     * 监控指标名称，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]，  其它会返回err
     * Required:true
     */
    @Required
    private String metric;

    /**
     * 数据维度，数据类型为map类型，key\value均为string类型，支持最少一个，最多五个标签，总长度不大于255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     * Required:true
     */
    @Required
    private Object dimensions;

    /**
     * 上报数据点的时间戳,只支持10位，秒级时间戳，不能写入过去30天的时间
     * Required:true
     */
    @Required
    private Long timestamp;

    /**
     * 数据上报类型，1为原始值，2为聚合数据。当上报聚合数据时，建议为60s的周期时行上报、否则无法正常查询
     * Required:true
     */
    @Required
    private Integer type;

    /**
     * 指标值集合，数据类型必须为map类型，key\value均为string类型，key为数据类型，value为数据值，当type&#x3D;1时，key只能为”value”，上报的是原始值，当type&#x3D;2时，K的值可以为&quot;avg&quot;,&quot;sum&quot;,&quot;last&quot;,&quot;max&quot;,&quot;min&quot;,&quot;count&quot;，只支持以上类型，否则会报错，value内容为整型或浮点型数字，value最大值为9223372036854775807，count只支持&gt;&#x3D;0的数
     * Required:true
     */
    @Required
    private Object values;


    /**
     * get 命名空间 ，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @return
     */
    public String getNamespace() {
        return namespace;
    }

    /**
     * set 命名空间 ，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @param namespace
     */
    public void setNamespace(String namespace) {
        this.namespace = namespace;
    }

    /**
     * get 监控指标名称，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]，  其它会返回err
     *
     * @return
     */
    public String getMetric() {
        return metric;
    }

    /**
     * set 监控指标名称，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]，  其它会返回err
     *
     * @param metric
     */
    public void setMetric(String metric) {
        this.metric = metric;
    }

    /**
     * get 数据维度，数据类型为map类型，key\value均为string类型，支持最少一个，最多五个标签，总长度不大于255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @return
     */
    public Object getDimensions() {
        return dimensions;
    }

    /**
     * set 数据维度，数据类型为map类型，key\value均为string类型，支持最少一个，最多五个标签，总长度不大于255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @param dimensions
     */
    public void setDimensions(Object dimensions) {
        this.dimensions = dimensions;
    }

    /**
     * get 上报数据点的时间戳,只支持10位，秒级时间戳，不能写入过去30天的时间
     *
     * @return
     */
    public Long getTimestamp() {
        return timestamp;
    }

    /**
     * set 上报数据点的时间戳,只支持10位，秒级时间戳，不能写入过去30天的时间
     *
     * @param timestamp
     */
    public void setTimestamp(Long timestamp) {
        this.timestamp = timestamp;
    }

    /**
     * get 数据上报类型，1为原始值，2为聚合数据。当上报聚合数据时，建议为60s的周期时行上报、否则无法正常查询
     *
     * @return
     */
    public Integer getType() {
        return type;
    }

    /**
     * set 数据上报类型，1为原始值，2为聚合数据。当上报聚合数据时，建议为60s的周期时行上报、否则无法正常查询
     *
     * @param type
     */
    public void setType(Integer type) {
        this.type = type;
    }

    /**
     * get 指标值集合，数据类型必须为map类型，key\value均为string类型，key为数据类型，value为数据值，当type&#x3D;1时，key只能为”value”，上报的是原始值，当type&#x3D;2时，K的值可以为&quot;avg&quot;,&quot;sum&quot;,&quot;last&quot;,&quot;max&quot;,&quot;min&quot;,&quot;count&quot;，只支持以上类型，否则会报错，value内容为整型或浮点型数字，value最大值为9223372036854775807，count只支持&gt;&#x3D;0的数
     *
     * @return
     */
    public Object getValues() {
        return values;
    }

    /**
     * set 指标值集合，数据类型必须为map类型，key\value均为string类型，key为数据类型，value为数据值，当type&#x3D;1时，key只能为”value”，上报的是原始值，当type&#x3D;2时，K的值可以为&quot;avg&quot;,&quot;sum&quot;,&quot;last&quot;,&quot;max&quot;,&quot;min&quot;,&quot;count&quot;，只支持以上类型，否则会报错，value内容为整型或浮点型数字，value最大值为9223372036854775807，count只支持&gt;&#x3D;0的数
     *
     * @param values
     */
    public void setValues(Object values) {
        this.values = values;
    }


    /**
     * set 命名空间 ，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @param namespace
     */
    public MetricData namespace(String namespace) {
        this.namespace = namespace;
        return this;
    }

    /**
     * set 监控指标名称，长度不超过255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]，  其它会返回err
     *
     * @param metric
     */
    public MetricData metric(String metric) {
        this.metric = metric;
        return this;
    }

    /**
     * set 数据维度，数据类型为map类型，key\value均为string类型，支持最少一个，最多五个标签，总长度不大于255字节，只允许英文、数字、下划线_、点., [0-9][a-z] [A-Z] [. _ ]， 其它会返回err
     *
     * @param dimensions
     */
    public MetricData dimensions(Object dimensions) {
        this.dimensions = dimensions;
        return this;
    }

    /**
     * set 上报数据点的时间戳,只支持10位，秒级时间戳，不能写入过去30天的时间
     *
     * @param timestamp
     */
    public MetricData timestamp(Long timestamp) {
        this.timestamp = timestamp;
        return this;
    }

    /**
     * set 数据上报类型，1为原始值，2为聚合数据。当上报聚合数据时，建议为60s的周期时行上报、否则无法正常查询
     *
     * @param type
     */
    public MetricData type(Integer type) {
        this.type = type;
        return this;
    }

    /**
     * set 指标值集合，数据类型必须为map类型，key\value均为string类型，key为数据类型，value为数据值，当type&#x3D;1时，key只能为”value”，上报的是原始值，当type&#x3D;2时，K的值可以为&quot;avg&quot;,&quot;sum&quot;,&quot;last&quot;,&quot;max&quot;,&quot;min&quot;,&quot;count&quot;，只支持以上类型，否则会报错，value内容为整型或浮点型数字，value最大值为9223372036854775807，count只支持&gt;&#x3D;0的数
     *
     * @param values
     */
    public MetricData values(Object values) {
        this.values = values;
        return this;
    }


}